...
namespace ControlLogic
{
    public class MainLogic
    {
        ...
        */
        ...
        public static void InCritical(Trains train, bool fintrack)
        {
            ...
            AddReservedSections("0x25", "id5");
            AddReservedSections("0x25", "id105");
        ...
        public static void AddCurrentDrain(string id, string section)
        {
            currentDrain.Add(new CurrentDrain { TrainIdDrain = id, Section = section });
        }

        public static void AddReservedSections(string id, string section)
        {
        ...
        public static void addNewTrainDataFromClient(string name, string currentPosition, string speed, string direction, string final)
        {
            ...
            foreach (Trains train in trainsList)
            {
                if (train.name == name)
                {
                    string orientation = train.direction;
                    ...
                    */
                    ...
                    var toCircuit = xdoc.Descendants("fromCircuit")
                        .Where(fc => fc.Attribute("id")?.Value == train.circuit)
                        .Descendants("toCircuit")
                        ...
                        foreach (XElement element in fromElements)
                        {
                            foreach (XElement toElement in element.Elements("to"))
                            {
                                ...
                                if (getRoad.Value == toCircuitName)
                                {
                                    ...
                                    var getCircuit = toElement.Element("circuit");
                                    ...
                                    if (((reserveSections.Contains(train.lastPosition)) && train.direction != direction) ||
                                        (!reserveSections.Contains(train.lastPosition) && train.direction == direction))
                                    {
                                        /*
                                         * prepared for switches to add
                                        var reserveSwitches = toElement.Element("switches")
                                        ...
                                        */
                                        ...
                                        train.circuit = getCircuit.Value;
                                    }
        ...
        public static IEnumerable<XElement> getFromCircuit(Trains train)
        {
            return xdoc.Descendants("fromCircuit")
                          .Where(e => (string)e.Attribute("id") == train.circuit);
    ...
    {
        public string TrainIdDrain { get; set; }
    ...
    {
        public string TrainIdReserved { get; set; }
        public string Section { get; set; }
